...
const CartItem = (props) => {
  ...
  const addToCart = (e) => {
    ...
    let isSameItemExists = state.orderlist?.find(
      (item) => item.data.name === e.name
    );
    let numOfItems = 0;
    ...
    if (isSameItemExists) {
      store.dispatch({
        type: "ADD_ALREADY",
        payload: { data: e, dishcount: numOfItems + 1 },
      });
    } else {
  ...
  const removeItemFromCart = (e) => {
    console.log("E3",e)
    store.dispatch({ type: "REMOVE", payload: e });
  };
  ...
  return (
    <div className="food-item pb-3">
      ...
      <div>Amount : {props.dishcount}</div>
      ...
      <span className="minus">
        <button onClick={() => removeItemFromCart(props)}>-</button>
      </span>
    </div>
  );
};
